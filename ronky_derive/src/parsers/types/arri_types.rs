...
impl ArriTypesParser {
    fn convert(ty: &Type, identifier: &proc_macro2::Ident) -> TokenStream {
        // TODO: I feel like we can find a way where we don't have to repeat the quote macro
        // constantly?
        match identifier.to_string().as_str() {
            "String" => quote!(ronky::TypeSchema::new(ronky::Types::String)).into(),
            "bool" => quote!(ronky::TypeSchema::new(ronky::Types::Boolean)).into(),
            "f32" => quote!(ronky::TypeSchema::new(ronky::Types::Float32)).into(),
            "f64" => quote!(ronky::TypeSchema::new(ronky::Types::Float64)).into(),
            "i8" => quote!(ronky::TypeSchema::new(ronky::Types::Int8)).into(),
            "u8" => quote!(ronky::TypeSchema::new(ronky::Types::Uint8)).into(),
            "i16" => quote!(ronky::TypeSchema::new(ronky::Types::Int16)).into(),
            "u16" => quote!(ronky::TypeSchema::new(ronky::Types::Uint16)).into(),
            "i32" => quote!(ronky::TypeSchema::new(ronky::Types::Int32)).into(),
            "u32" => quote!(ronky::TypeSchema::new(ronky::Types::Uint32)).into(),
            "i64" => quote!(ronky::TypeSchema::new(ronky::Types::Int64)).into(),
            "u64" => quote!(ronky::TypeSchema::new(ronky::Types::Uint64)).into(),
            _ => quote!(<#ty as ronky::Exportable>::export()).into(),
        }
    }

    fn parse_arguments(ty: &Type, args: &Punctuated<GenericArgument, Comma>) -> TokenStream {
        if args.is_empty() {
        ...
        ArriTypesParser::convert(ty, &repr)
    }

    fn parse_segments(ty: &Type, segments: &Punctuated<PathSegment, PathSep>) -> TokenStream {
        ...
        match segment.ident.to_string().as_str() {
            "Option" => {
                ...
                return Self::parse_arguments(ty, &args.args);
            }
            _ => ArriTypesParser::convert(ty, &segment.ident),
        }
    }
}

impl TypeParser for ArriTypesParser {
    fn parse(ty: &Type, path: &Path) -> TokenStream {
        Self::parse_segments(&ty, &path.segments)
    }
}
